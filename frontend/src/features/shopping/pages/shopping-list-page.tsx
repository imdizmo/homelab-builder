...
import { Printer, ShoppingCart, Globe, ExternalLink, Package, Cpu, HardDrive, Network, Wifi, Server, CheckCircle2, Eye, EyeOff } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
export default function ShoppingListPage() {
  ...
  const totalCost = items.reduce((acc, item) => {
      const cheapestNew = item.offers.filter(o => o.condition === 'new').sort((a, b) => a.price - b.price)[0]
    ...
    <div className="space-y-6 print:space-y-4 max-w-5xl mx-auto py-8 px-6">
      ...
      {pendingBuy && (
          <BuyConfirmDialog
              ...
              onBought={() => { markAsBought(pendingBuy.itemName); setPendingBuy(null) }}
      ...
      <div className="flex items-center justify-between rounded-xl border bg-primary/5 px-6 py-4">
          <div>
              ...
              <p className="text-4xl font-bold text-primary mt-0.5">
                  ~{totalCost.toLocaleString()} <span className="text-xl font-normal text-muted-foreground">{currency}</span>
              </p>
          </div>
          <div className="text-right text-sm">
              <div className="flex items-center gap-2 justify-end mb-1">
                  {boughtCount > 0 && (
                      <Badge variant="default" className="bg-green-600 hover:bg-green-700 cursor-pointer" onClick={() => setShowBought(!showBought)}>
                          <CheckCircle2 className="h-3 w-3 mr-1" /> {boughtCount} bought
                      </Badge>
                  )}
                  <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => setShowBought(!showBought)}>
                      {showBought ? <EyeOff className="h-3 w-3 mr-1" /> : <Eye className="h-3 w-3 mr-1" />}
                      {showBought ? 'Hide bought' : 'Show bought'}
                  </Button>
              </div>
              <p className="text-xs text-muted-foreground">{items.length} item{items.length !== 1 ? 's' : ''} remaining</p>
          ...
          <Table>
              ...
              <TableBody>
                  {items.map((item, index) => {
                      ...
                      return (
                          <TableRow key={index} className={`hover:bg-muted/30 transition-colors align-top ${isBought ? 'opacity-50' : ''}`}>
                              {/* Component */}
                              <TableCell className="pl-6 py-5 align-top">
                                  <div className="flex items-start gap-3">
                                      <div className={`mt-0.5 p-2 rounded-lg shrink-0 ${isBought ? 'bg-green-500/20 text-green-600' : 'bg-primary/10 text-primary'}`}>
                                          {isBought ? <CheckCircle2 className="h-4 w-4" /> : <Icon className="h-4 w-4" />}
                                      </div>
                                      <div className="flex flex-col gap-1.5">
                                          <span className={`font-semibold leading-tight ${isBought ? 'line-through text-muted-foreground' : ''}`}>{item.name}</span>
                                          {item.spec && (
                                              <span className="text-xs text-muted-foreground leading-snug">{item.spec}</span>
                                          )}
                                          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                              <Badge
                                                  variant={isBought ? 'default' : item.priority === 'essential' ? 'default' : 'outline'}
                                                  className={`text-[10px] h-4 px-1.5 rounded-sm ${isBought ? 'bg-green-600 hover:bg-green-700' : ''}`}
                                              >
                                                  {isBought ? '✓ Bought' : item.priority === 'essential' ? 'Required' : item.priority === 'manual' ? 'Your Pick' : 'Optional'}
                                              </Badge>
                                              <Badge variant="secondary" className="text-[10px] h-4 px-1.5 rounded-sm font-normal">
                                                  {item.category}
                                              </Badge>
                                              {isBought && (
                                                  <button
                              ...
                              <TableCell className="py-5 pr-6 align-top">
                                  <div className="flex flex-col gap-2">
                                      {newOffers.map((offer, i) => (
                                          <div key={i} className="flex items-center justify-between gap-3 rounded-lg border bg-background/60 px-3 py-2">
                                              <div className="flex items-center gap-2 min-w-0">
                                                  <span className="text-[10px] font-bold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0">New</span>
                                      ...
                                      {usedOffers.map((offer, i) => (
                                          <div key={i} className="flex items-center justify-between gap-3 rounded-lg border border-dashed border-amber-300/50 bg-amber-50/30 dark:bg-amber-900/10 px-3 py-2">
                                              <div className="flex items-center gap-2 min-w-0">
                                                  <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0">Used</span>
                                                  <span className="text-sm font-medium truncate">{offer.store}</span>
                                              </div>
                                              <div className="flex items-center gap-3 shrink-0">
                      ...
                      <TableRow>
                          <TableCell colSpan={2} className="py-3 px-6 bg-green-500/5 border-t">
                              <p className="text-xs text-green-600 dark:text-green-400 font-medium flex items-center gap-1.5">
                                  ...
                                  {boughtCount} item{boughtCount !== 1 ? 's' : ''} already purchased — shown above with strikethrough
                              </p>
                          </TableCell>
                      </TableRow>
                  )}
