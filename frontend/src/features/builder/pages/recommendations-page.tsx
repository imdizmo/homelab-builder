  ...
  if (hardwareNodes.length === 0 || serviceIds.length === 0) {
    return (
        <div className="flex flex-col items-center justify-center p-12 text-center h-[calc(100vh-4rem)]">
            ...
            <p className="text-muted-foreground mb-8">Please add hardware nodes and deploy some services onto them in the Visual Builder first.</p>
      ...
      <div className="border-b bg-card z-10 shrink-0">
          <div className="flex items-center justify-between p-4 max-w-7xl mx-auto w-full">
            ...
            <div className="flex gap-2">
                 <Button variant={showInsights ? "secondary" : "default"} size="sm" onClick={() => setShowInsights(!showInsights)}>
          ...
          <div className={cn(
              ...
              showInsights ? "max-h-[60vh] opacity-100 py-4" : "max-h-0 opacity-0 overflow-hidden py-0 border-none"
          )}>
               {isLoading && (
                       ...
                       <div className="space-y-4">
                           <Card>
                                <CardHeader className="py-3">
                                    <CardTitle className="text-sm">Minimum Requirements for Selected Services</CardTitle>
                                </CardHeader>
                           ...
                           <Card className="bg-primary/5 border-primary/20">
                                <CardHeader className="py-3">
                                    <CardTitle className="text-sm text-primary">Compatibility & Rationale</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-2 pb-3 text-sm">
                                    ...
                                    <div className="flex flex-col gap-2 mt-4 text-xs">
                                        {recResponse.insights.slice(0, 3).map(insight => (
                                            <div key={insight.name} className="flex gap-2 items-start text-muted-foreground">
                                                <CheckCircle2 className="h-3 w-3 mt-0.5 text-primary shrink-0" />
                                                <span><strong>{insight.name}:</strong> {insight.note}</span>
                                            </div>
                                        ))}
                                    </div>
                                </CardContent>
                                   ...
                                   {spec.hardware_matches.map((hw: CatalogComponent) => (
                                       <Card key={hw.id} className="hover:border-primary/50 transition-colors">
                                           <CardContent className="p-4 flex items-center justify-between gap-4">
                                               <div>
                                                   <div className="font-semibold">{hw.brand} {hw.model}</div>
                                                   <div className="text-xs text-muted-foreground capitalize">{hw.category}</div>
                                               </div>
                                               <div className="text-right flex-shrink-0">
                                                    <div className="font-bold text-primary">~{hw.price_est} {hw.currency}</div>
                                                    {hw.buy_urls && hw.buy_urls.length > 0 && (
                                                        ...
                                                        >
                                                            Buy on {hw.buy_urls[0].store} <ExternalLink className="w-3 h-3 ml-1" />
                                                        </a>
                                                    )}
                                               </div>
                                           </CardContent>
