...
import { ClipboardList, CheckCircle2, ChevronDown, ChevronRight, Server, Globe, Download, Terminal, Network, Shield } from "lucide-react"
...
export default function ChecklistPage() {
  ...
  const hasServer = hardwareNodes.some(n => n.type === 'server' || n.type === 'pc' || n.type === 'minipc' || n.type === 'sbc');
  ...
  hardwareNodes.forEach((node) => {
      if (node.type === 'server' || node.type === 'pc' || node.type === 'minipc' || node.type === 'sbc') {
          node.vms?.forEach((vm) => deployedServices.add(vm.name.toLowerCase()));
      }
  });

  const hasDockerServices = deployedServices.size > 0;
  const hasProxy = deployedServices.has('nginx proxy manager') || deployedServices.has('traefik') || deployedServices.has('caddy');
  ...
  const steps = useMemo(() => {
    ...
    if (hasRouter || hasSwitch) {
        s.push({
            ...
            icon: Network,
            items: [
                ...(hasRouter ? [{ text: 'Configure your primary router/firewall interfaces (WAN/LAN).' }] : []),
    ...
    s.push({
        ...
        items: [
            ...
            { text: 'Create a non-root user and add them to the sudo group:', code: 'adduser username\nusermod -aG sudo username' },
            { text: 'Generate SSH keys on your personal PC and copy them to the server:', code: 'ssh-copy-id username@SERVER_IP' },
            { text: 'Disable password authentication in SSH:', code: 'sudo nano /etc/ssh/sshd_config\n# Set PasswordAuthentication no\n# Set PermitRootLogin prohibit-password\nsudo systemctl restart ssh' },
        ]
    });
    ...
    if (hasDockerServices) {
        s.push({
            ...
            items: [
                ...
                { text: 'Install Docker and Docker Compose:', code: 'curl -fsSL https://get.docker.com -o get-docker.sh\nsudo sh get-docker.sh\nsudo usermod -aG docker $USER' },
      ...
      return (
          <div className="flex flex-col items-center justify-center p-12 text-center border rounded-lg bg-muted/20 min-h-[400px]">
              ...
              <p className="text-muted-foreground mb-6 max-w-md">
                  Your setup instructions will be dynamically generated here based on the hardware and services you add in the Visual Builder.
              </p>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-2">
            ...
            <p className="text-muted-foreground max-w-2xl">
                A personalized step-by-step technical guide based on the <Badge variant="secondary" className="mx-1">{hardwareNodes.length} devices</Badge> 
                and <Badge variant="secondary" className="mx-1">{deployedServices.size} services</Badge> in your current project.
            </p>
        </div>
